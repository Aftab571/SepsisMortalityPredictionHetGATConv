...
class GAT(torch.nn.Module):
    def __init__(self, hidden_channels, heads):
        ...
        self.conv1 = GATConv((-1,-1), 16,edge_dim=1,add_self_loops=False, heads=2)  # TODO  64
        self.conv2 = GATConv(32, 2,edge_dim=1,add_self_loops=False, heads=1)
...
def main():
    ...
    df_labs.to_csv('removedAllStr.csv')
    end_time = time.time()
    ...
    for aggr in ['sum']:
        ...
        for i in mask_list:
            ...
            data['Admission'].test_mask = test_mask.to(device)
            ...
            # df_demo['randNumCol'] = np.random.randint(0, 100, df_demo.shape[0])
            ...
            # data['Admission', 'has_same_demo', 'Demography'].edge_attr = torch.tensor(df_demo[['randNumCol']].values.tolist(), dtype=torch.float).contiguous().to(device)
            ...
            # data['Demography', 'rev_same_demo', 'Admission'].edge_attr = torch.tensor(df_demo[['randNumCol']].values.tolist(), dtype=torch.float).contiguous().to(device)
            ...
            loader = NeighborLoader(data, input_nodes=('Admission', data['Admission'].train_mask), batch_size=2, shuffle=False, num_neighbors={key: [30] * 2 for key in data.edge_types})  #sampler=sampler
